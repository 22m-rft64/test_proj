...
# JSON 응답의 한글을 \uXXXX로 이스케이프하지 않는다. curl 출력이 그대로 읽힌다.
app.json.ensure_ascii = False

# ---------------------------------------------------------------------------
# 강의용 취약점 스위치. VULN=1 로 켜면 아래 네 군데가 "개발자가 빼먹은" 버전으로
# 동작한다. 끄면(기본값) 전부 정상 코드다. 실습 중에 켰다 껐다 하면서
# 같은 화면이 어떻게 뚫리고 어떻게 막히는지 비교하는 용도다.
#
#   A. IDOR          /memos/<id>      URL 숫자    소유권 검사 누락
#   B. 쿠키 신뢰     /admin           쿠키        권한을 클라이언트가 들고 있음
#   C. SQL 인젝션    /search?q=       폼          검색어를 SQL에 문자열로 이어붙임
#   D. 저장형 XSS    메모 본문        폼          출력 이스케이프 해제
#
# 절대 이 플래그를 켠 채로 공개 서버에 두지 마라.
# ---------------------------------------------------------------------------
VULN = os.environ.get("VULN") == "1"
...
def inject_user():
    ...
    return {"user": current_user(), "vuln": VULN}
...
def admin_required(view):
    ...
    def wrapped(*a, **kw):
        ...
        if u is None:
            ...
            return redirect(url_for("login"))
        if VULN:
            # [취약점 B] 권한 판단의 근거가 서버 DB(u["is_admin"])가 아니라
            # 클라이언트가 보낸 쿠키다. 브라우저 개발자도구에서 값 한 글자만
            # 고치면 관리자가 된다. 쿠키는 "사용자 입력"이지 신원증명이 아니다.
...
def own_memo_or_404(memo_id):
    ...
    if VULN:
        # [취약점 A] IDOR. "로그인했으니 통과"까지만 확인하고, 이 메모가
        # 정말 내 것인지는 묻지 않는다. AND user_id = ? 한 조각이 통째로 빠졌다.
        # 주소창의 숫자가 곧 남의 데이터로 가는 열쇠가 된다.
        memo = get_db().execute(
...
{% block body %}
  {% if memos %}
    ...
    <form class="searchbar" method="get" action="{{ url_for('search') }}">
      <input name="q" placeholder="제목으로 검색" aria-label="메모 검색">
...
{% block body %}
  ...
  {# [취약점 D] VULN이면 |safe 로 이스케이프를 꺼서 본문의 HTML/스크립트가 그대로 실행된다. #}
  <p class="doc">{% if vuln %}{{ memo['body']|safe }}{% else %}{{ memo['body'] }}{% endif %}</p>
...
{% block body %}
  <form class="searchbar" method="get" action="{{ url_for('search') }}">
    <input name="q" value="{{ q }}" placeholder="제목으로 검색" autofocus aria-label="메모 검색">
    <button class="btn btn--sm" type="submit">검색</button>
  </form>
  {% if q %}
    <p class="meta">"{{ q }}" 검색 결과 {{ memos|length }}개</p>
  {% endif %}
  {% if memos %}
    <ul class="list">
      {% for m in memos %}
  ...
  {% elif q %}
    <p class="empty">일치하는 메모가 없습니다.</p>
  {% endif %}
  <div class="actions"><a class="btn btn--sm" href="{{ url_for('index') }}">목록</a></div>
{% endblock %}
""",
})
...
def search():
    ...
    if q:
        if VULN:
            # [취약점 C] SQL 인젝션. 사용자가 친 글자가 따옴표 안으로 그대로
            # 들어가서, 검색어가 아니라 질의문의 일부가 된다.
            # ' OR '1'='1  한 줄이면 user_id 조건까지 통째로 무력화된다.
            sql = (
                "SELECT id, title, updated_at FROM memos "
                "WHERE user_id = %s AND title LIKE '%%%s%%' "
                "ORDER BY id DESC" % (session["user_id"], q)
            )
    ...
    return render_template("search.html", q=q, memos=rows)
...
# ---------- JSON API ----------
# 화면(HTML) 대신 데이터(JSON)를 주고받는 입구다. 브라우저가 아니라 curl,
# 모바일 앱, 다른 서버가 이쪽으로 붙는다. 인증은 웹 화면과 똑같은 세션 쿠키를
# 그대로 쓴다 — 로그인이란 결국 쿠키 한 장이라는 걸 여기서 눈으로 본다.

def note_json(row):
    """DB 한 행을 JSON으로 내보낼 모양으로 바꾼다."""
    return {
        "id": row["id"],
        "title": row["title"],
        "body": row["body"],
...
def api_login_required(view):
    """웹 화면은 로그인 페이지로 리다이렉트하지만, API는 401 JSON을 준다.
    curl로 부르는 쪽은 HTML 로그인 폼을 받아도 해석할 수 없기 때문이다."""
    @functools.wraps(view)
    def wrapped(*a, **kw):
        if current_user() is None:
...
def api_notes():
    ...
    return jsonify(notes=[note_json(r) for r in rows], count=len(rows))


@app.route("/api/notes/<int:memo_id>")
@api_login_required
def api_note(memo_id):
    if VULN:
        # [취약점 E] 웹 화면 쪽 소유권 검사는 고쳐놓고 API만 빼먹는, 현장에서
        # 가장 흔한 사고. 같은 데이터에 입구가 두 개면 검사도 두 군데 해야 한다.
        row = get_db().execute(
            "SELECT * FROM memos WHERE id = ?", (memo_id,)
        ).fetchone()
    else:
    ...
    if row is None:
        return jsonify(error="없는 메모입니다."), 404
...
if __name__ == "__main__":
    # 컨테이너 안에서 Caddy가 app:8000 으로 붙는다. 로컬 단독 실행은 PORT로 바꿔라.
